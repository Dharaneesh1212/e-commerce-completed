...
const Cart = ({ cart, setCart }) => {
  ...
  return (
    <div className="w-full flex flex-col justify-center items-center bg-white ">
      <div className="w-[90rem] flex flex-row justify-evenly font-bold text-xl  mt-20 border-b-4 pb-8 text-black">
      ...
      <div className="w-[90rem] flex flex-col justify-evenly font-bold text-xl gap-4">
        {cart.map((item) => {
          ...
          return (
            ...
            >
              <span className="text-xl font-semibold w-[8rem] text-black">
                {item.name}
              </span>
              <span>
                <img src={item.imgUrl} alt={item.name} className="w-20 h-20" />
              </span>
              <span className="text-xl font-semibold flex items-center justify-center gap-2 text-black w-[8rem] text-center">
                <button
                  onClick={() => {
                    addQuantity(item.id);
                  }}
                  className="bg-black rounded  w-5 h-7 text-white"
                >
                  +
                </button>
                {item.quantity}
              ...
              <span className="w-[8rem] text-black">
